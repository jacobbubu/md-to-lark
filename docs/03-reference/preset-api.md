...
CLI 现在支持重复传 `--preset`，程序化调用支持 `presetPaths` 数组。

执行规则是：

1. 严格按给定顺序执行
2. 上一个 preset 的输出作为下一个 preset 的输入
3. 任一 preset 报错，整次发布直接失败

程序化调用优先级：

1. 如果传了 `presetPaths`，优先使用它
2. 如果没有 `presetPaths`，再回退到兼容字段 `presetPath`
